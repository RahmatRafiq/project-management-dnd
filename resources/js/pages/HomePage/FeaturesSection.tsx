...
export default function FeaturesSection() {
  ...
  return (
    <section id="features" className="relative py-20 bg-gray-100 overflow-hidden">
      <div className="absolute inset-0 bg-gray-100 bg-opacity-30 backdrop-blur-md pointer-events-none" />

      <div className="relative container mx-auto text-center">
        <h2 className="text-5xl font-extrabold text-gray-800">Build Perfect Docs</h2>
        ...
        <Tab.Group selectedIndex={selectedGroup} onChange={setSelectedGroup}>
          <Tab.List className="mt-12 inline-flex space-x-2 bg-gray-200 p-1 rounded-full">
            {groups.map((_, idx) => (
              <Tab
                key={idx}
                className={({ selected }) =>
                ...
                >
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                    {group.items.map((feat) => (
                      <div
                        key={feat.id}
                        className="flex flex-col w-full bg-white rounded-xl shadow-lg overflow-hidden"
                      >
                        <img
                          src={feat.image}
                          alt={feat.title}
